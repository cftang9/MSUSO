...
for(b in 1:B){
  ...
  M34s = MP(R34,nv[3],nv[4])
  
  Ds = rbind(D12s,D23s,D34s)
  Ms = rbind(M12s,M23s,M34s)
  ...
  constps = log(log(cs))*1# 1.201; #log(log(cs)); #log(log(cs)) #1/2#sqrt(log(log(cs))/2) # 1.201/1.353#log(log(cs))/2; #log(log(cs))/2
