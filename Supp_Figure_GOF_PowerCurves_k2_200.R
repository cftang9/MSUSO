...
# save(file="PCsk2n200.Rdata",
#      pSks_C1_200, pWks_C1_200, pBon_C1_200,
#      pSks_C3_200, pWks_C3_200, pBon_C3_200)

#pdf("Supp_Figure_GOF_PowerCurves_k2_200.pdf", width=19.5, heigh=13)

#png("Supp_Figure_GOF_PowerCurves_k2_200.png", width=1872, heigh=1248)

par(mfrow=c(2,3))
par(mar=c(2,2,2,2)) 
plot((Cases-1)/10, pSks_C1_200[,1], type="l",ylim=c(0,1)); 
lines((Cases-1)/10, pBon_C1_200[,1],lty=2); 
abline(h=0.05,lty=3); 
plot((Cases-1)/10, pSks_C1_200[,2], type="l",ylim=c(0,1)); 
lines((Cases-1)/10, pBon_C1_200[,2],lty=2); 
abline(h=0.05,lty=3); 
plot((Cases-1)/10, pSks_C1_200[,3], type="l",ylim=c(0,1)); 
lines((Cases-1)/10, pBon_C1_200[,3],lty=2); 
abline(h=0.05,lty=3); 
plot((Cases-1)/10, pSks_C3_200[,1], type="l",ylim=c(0,1)); 
lines((Cases-1)/10, pBon_C3_200[,1],lty=2); 
abline(h=0.05,lty=3); 
plot((Cases-1)/10, pSks_C3_200[,2], type="l",ylim=c(0,1)); 
lines((Cases-1)/10, pBon_C3_200[,2],lty=2); 
abline(h=0.05,lty=3); 
plot((Cases-1)/10, pSks_C3_200[,3], type="l",ylim=c(0,1)); 
