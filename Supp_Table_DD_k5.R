...
Table.DD.K5.RR.200 = array(,c(4,9)); 
for(j in 1:4){
  ...
  exact.jump=c(poi(case=j,k=3),0,0);
...
for(j in 1:16){
  ...
  Table.DD.K5.MN.200[j,] = c(C1,C2,C3); 
}
...
Table.DD.K5.RR.400 = array(,c(4,9)); 
for(j in 1:4){
  Jump.p1 = array(NA,c(B,(k-1))); Jump.p2 = array(NA,c(B,(k-1))); Jump.ps = array(NA,c(B,(k-1))); M1s.data = array(NA,c(B,(k-1))); M2s.data = array(NA,c(B,(k-1))); Mss.data = array(NA,c(B,(k-1))); 
  set.seed(02142021)
  for(b in 1:B){
  ...
  Table.DD.K5.RR.400[j,] = c(C1,C2,C3); 
}
...
for(j in 1:16){
  ...
  Table.DD.K5.MN.400[j,] = c(C1,C2,C3); 
}
