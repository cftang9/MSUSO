...
nv = c(200,200); 
B = 1000; BB = 1000; 
Table.GOF.K2.MN.200 = array(,c(7,9)); 
start = Sys.time();
for(j in 1:7){
  power.Sks = array(0,3); power.Wks = array(0,3); power.Bon = array(0,3); 
  ...
  Table.GOF.K2.MN.200[j,] = c(power.Sks[1],power.Wks[1],power.Bon[1],   power.Sks[2],power.Wks[2],power.Bon[2],   power.Sks[3],power.Wks[3],power.Bon[3])
}
...
nv = c(400,400);
B = 1000; BB = 1000; 
Table.GOF.K2.MN.400 = array(,c(7,9)); 
start = Sys.time();
for(j in 1:7){
  power.Sks = array(0,3); power.Wks = array(0,3); power.Bon = array(0,3); 
  ...
  Table.GOF.K2.MN.400[j,] = c(power.Sks[1],power.Wks[1],power.Bon[1],   power.Sks[2],power.Wks[2],power.Bon[2],   power.Sks[3],power.Wks[3],power.Bon[3])
}

# save(file="SPsk2.Rdata", Table.GOF.K2.RR.200, Table.GOF.K2.RR.400, Table.GOF.K2.MN.200, Table.GOF.K2.MN.400)